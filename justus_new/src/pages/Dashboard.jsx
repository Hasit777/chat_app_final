...
export default function Dashboard() {
  ...
  async function loadDashboard() {
    try {
      ...
      // Count unread messages (not sent by me, not yet read by me)
      const allMsgsSnap = await getDocs(collection(db, 'messages'))
      const unread = allMsgsSnap.docs.filter(d => {
        const data = d.data()
    ...
    <div className={styles.page}>
      ...
      <div className={styles.container}>
        ...
        {notifPermission === 'default' && (
          <div className={styles.pinHint} onClick={requestPermission}>
            <span>🔔</span>
            <span>Turn on notifications so you never miss a message</span>
          </div>
        )}
